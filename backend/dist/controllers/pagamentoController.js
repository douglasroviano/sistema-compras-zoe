...
exports.createPagamentoPorCliente = exports.deletePagamento = exports.updatePagamento = exports.createPagamento = exports.getPagamento = exports.getPagamentos = void 0;
...
const getPagamentos = async (req, res) => {
    try {
        // Buscar pagamentos
        const { data: pagamentos, error: pagamentosError } = await supabaseClient_1.supabase
            .from('pagamentos')
            .select('*');
        if (pagamentosError) {
...
const getPagamento = async (req, res) => {
    const { id } = req.params;
    const { data, error } = await supabaseClient_1.supabase.from('pagamentos').select('*').eq('id', id).single();
    if (error) {
...
const createPagamento = async (req, res) => {
    try {
        ...
        const { data, error } = await supabaseClient_1.supabase.from('pagamentos').insert([pagamento]).select().single();
        if (error) {
...
const updatePagamento = async (req, res) => {
    try {
        ...
        const { data: pagamentoAtual } = await supabaseClient_1.supabase
            .from('pagamentos')
            .select('venda_id')
            .eq('id', id)
            .single();
        const { data, error } = await supabaseClient_1.supabase.from('pagamentos').update(pagamento).eq('id', id).select().single();
        if (error) {
            res.status(400).json({ error: error.message });
            return;
        }
...
const deletePagamento = async (req, res) => {
    try {
        ...
        const { data: pagamento } = await supabaseClient_1.supabase
            .from('pagamentos')
            .select('venda_id')
            .eq('id', id)
            .single();
        const { error } = await supabaseClient_1.supabase.from('pagamentos').delete().eq('id', id);
        if (error) {
            res.status(400).json({ error: error.message });
            return;
        }
...
const createPagamentoPorCliente = async (req, res) => {
    try {
        ...
        const { data: pagamentosCriados, error: pagamentosError } = await supabaseClient_1.supabase
            .from('pagamentos')
...
exports.createPagamentoPorCliente = createPagamentoPorCliente;
