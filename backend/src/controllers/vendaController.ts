...
export const updateVenda = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const vendaData: any = req.body; // Usar any para permitir metodo_entrega
...
export const getLucroVendas = async (req: Request, res: Response) => {
  try {
...
export const createVendaComProdutos = async (req: Request, res: Response) => {
  try {
    ...
    if (produtos && produtos.length > 0) {
      const produtosParaCriar = produtos.map((produto: any) => ({
        ...produto,
        venda_id: vendaCriada.id // Associar ao ID da venda criada
      }));

      const { data: produtosCriados, error: produtosError } = await supabase
        .from('produtos_venda')
...
export const updateVendaComProdutos = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { vendaData, produtos } = req.body;
    const valorEntrada = vendaData.valor_pago || 0;
